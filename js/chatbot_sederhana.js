...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // <-- KUNCI API RAHASIA SUDAH DIHAPUS DARI SINI -->

  // Ganti URL untuk memanggil "perantara" amanmu di Vercel
  const API_URL = "/api/chat"; // <-- INI PERUBAHAN PENTING 1
  ...
  // --- Fungsi untuk mendapatkan respon dari AI (DIUBAH) ---
  async function getAiResponse(userQuestion) {
    // Pastikan KNOWLEDGE_BASE sudah dimuat (dari file chatbot_knowledge.js)
    if (typeof KNOWLEDGE_BASE === "undefined") {
      console.error("KNOWLEDGE_BASE is not loaded.");
      return "Maaf, basis pengetahuan saya sedang tidak termuat. Coba refresh halaman.";
    }

    try {
      // <-- INI PERUBAHAN PENTING 2: Mengirim request ke API perantaramu
      const response = await fetch(API_URL, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          userQuestion: userQuestion, // Kirim pertanyaan pengguna
          knowledgeBase: KNOWLEDGE_BASE, // Kirim basis pengetahuan
        }),
      });
      ...
      if (!response.ok) {
        ...
        return `Maaf, terjadi kesalahan dari server: ${errorData.error}`;
      }

      const data = await response.json();
      return data.answer; // Mengambil jawaban dari server perantaramu
    } catch (error) {
      console.error("Gagal menghubungi AI:", error);
      return "Terjadi kesalahan koneksi. Pastikan internet Anda stabil.";
    }
  ...
  const addMessageToWindow = (message, className, isLoading = false) => {
    ...
    chatWindow.appendChild(messageDiv);
    chatWindow.scrollTop = chatWindow.scrollHeight;
    return messageDiv; // Kembalikan elemen pesan
  };
